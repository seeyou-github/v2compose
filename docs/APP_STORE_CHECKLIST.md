...
- [ ] 统一 iOS 版本号来源，避免 `Info.plist` 与 build settings 不一致
  - 当前状态：
    - `iosApp/Configuration/Base.xcconfig` 中为 `MARKETING_VERSION = 1.0.1`
    - `iosApp/iosApp/Info.plist` 中为 `CFBundleShortVersionString = 1.0`
    - `iosApp/iosApp/Info.plist` 中为 `CFBundleVersion = 1`
- [ ] 确认本次发版版本号，例如 `1.0.1`
- [ ] 确认本次发版 build 号，例如 `2`
...
- [ ] 确认 `iosApp/iosApp/GoogleService-Info.plist` 已存在且内容正确
- [ ] 确认 Release Archive 时能正确拷贝 `GoogleService-Info.plist`
- [ ] 确认 Crashlytics dSYM 上传脚本在 Archive 后可正常执行
- [ ] 确认 Debug/Release 数据不会串到同一个 Firebase 项目里

## 4. 隐私与合规

- [ ] 添加 app 级 `PrivacyInfo.xcprivacy`
  - 当前状态：仓库中未找到该文件
- [ ] 核对实际使用的权限与系统能力
  - [x] Photos Add Only
...
- [ ] 确认 `NSPhotoLibraryAddUsageDescription` 文案准确
- [ ] 如果后续新增权限，补充对应 usage description
- [ ] 准备并上线可公开访问的 Privacy Policy 页面
  - 当前状态：仓库中未发现明确的隐私政策链接
- [ ] 准备 Support URL 或支持邮箱页面
  - 当前状态：仓库中未发现明确的支持链接
- [ ] 准备 Terms of Service 页面（建议）
- [ ] 在 App Store Connect 正确填写 App Privacy 问卷
  - [ ] 基于 Firebase Analytics 实际采集项填写 usage data / identifiers
  - [ ] 基于 Firebase Crashlytics 实际采集项填写 diagnostics
  - [ ] 不要仅依据 README 中的口头描述填写

## 5. 审核敏感功能说明

- [ ] 为审核准备 Review Notes
- [ ] 在 Review Notes 中说明以下能力的用途：
  - [ ] 自动签到
  ...
  - [ ] Firebase 仅用于崩溃与基础统计
- [ ] 如果审核需要登录，准备测试账号与密码
- [ ] 如果功能依赖特定环境或时机，补充复现步骤

## 6. 设备支持范围

- [ ] 确认是否真的要支持 iPad
  - 当前状态：`TARGETED_DEVICE_FAMILY = "1,2"`，即 iPhone + iPad
- [ ] 如果 **不支持 iPad**：改为仅 iPhone 并重新验证布局
- [ ] 如果 **支持 iPad**：
  - [ ] 补 iPad 截图
  - [ ] 验证 iPad 布局与交互
...
- [ ] 准备 App Store 截图
  ...
  - [ ] 如保留 iPad 支持，补充 iPad screenshots
- [ ] 准备 App 名称
- [ ] 准备副标题
- [ ] 准备关键词
- [ ] 准备中英文描述文案
...
5. [ ] 填 App Privacy 问卷草稿
6. [ ] 做一次完整 Release Archive 验证
7. [ ] 准备 Review Notes 并提交 TestFlight / App Review
