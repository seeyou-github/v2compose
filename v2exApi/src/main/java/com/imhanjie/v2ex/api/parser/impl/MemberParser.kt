...
object MemberParser : ParserMatcher {
    ...
    override fun parser(html: String): Any {
        ...
        val infoText = eMember?.selectFirst("span.gray")?.text() ?: ""
        val id = infoText.split("，").getOrNull(0)?.split(" ")?.getOrNull(2)?.toLongOrNull() ?: 0L
        val createTime = eMember?.selectFirst("span.gray")?.textNodes()?.getOrNull(0)?.text()?.split("，")?.getOrNull(1)?.replace("加入于 ", "") ?: ""
        ...
        if (eButton != null && eButton.size == 2) {
            isFollowing = eButton[0].attr("value") == "取消特别关注"
            isBlock = eButton[1].attr("value") == "Unblock"
            blockParamT = eButton[1].attr("onclick").split("=").last().split("'").getOrNull(0) ?: ""
        }

        var isMe = false
        try {
            isMe = userName == document.selectFirst("#Top")?.select("td")?.getOrNull(2)?.select("a")?.getOrNull(1)?.text()
